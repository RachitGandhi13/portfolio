...
export default function About() {
  return (
    <section id="about" className="bg-cream dark:bg-ink text-ink dark:text-cream">
      ...
      <div className="px-6 md:px-10 lg:px-14 pt-20 pb-0 flex items-center gap-4">
        ...
        <span className="text-[9px] font-mono text-ink/25 dark:text-cream/25 uppercase tracking-[0.45em]">About</span>
      </div>
      ...
      <div className="grid lg:grid-cols-[1fr_1fr] gap-0 mt-12">
        ...
        >
          <p
            className="font-display text-ink dark:text-cream uppercase leading-tight"
            style={{ fontSize: "clamp(28px, 3.5vw, 48px)" }}
          >
            Final-year CS at SRM Chennai.
          </p>

          <div className="space-y-4 text-[14px] text-ink/50 dark:text-cream/50 leading-[1.75] max-w-lg" style={{ textAlign: "justify" }}>
            <p>
      ...
      <motion.div
        {...enter(0.15)}
        className="px-6 md:px-10 lg:px-14 py-14 border-t border-ink/8 dark:border-cream/8"
      >
        <div className="space-y-0 divide-y divide-ink/6 dark:divide-cream/6">
          {skills.map((group) => (
            <div key={group.category} className="flex gap-8 items-baseline py-4">
              ...
              <div className="flex flex-wrap gap-x-5 gap-y-1">
                {group.items.map((item) => (
                  <span key={item} className="text-[12px] font-mono text-ink/40 dark:text-cream/40 hover:text-ink dark:hover:text-cream transition-colors">
                    {item}
                  </span>
                ))}
              </div>
